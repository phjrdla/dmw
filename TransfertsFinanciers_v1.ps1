...
from dual
/
select acc.LABEL||' ; '||acc.ACC_NUMBER||' ; '||to_char(m.AMOUNT)||' ; '||m.CRE_DEB||' ; '||nvl(cu.name,' ')||' ; '||ea.IBAN||' ; '||fo.POLICY_NUMBER "ligne"
...
column "ligne" format A140 trunc
ttitle '$reportOut'
select 'SOURCE_ACCOUNT ; TARGET_ACCOUNT ; QUANTITY_OR_AMOUNT ; EXECUTION_DATE ; SELECTED' "titres"
from dual
/
select payer.IBAN||' ; '||payee.IBAN||' ; '||to_char(eod.QUANTITY_OR_AMOUNT)||' ; '||to_char(co.EXECUTION_DATE)||' ; '||eod.DISPLAY "ligne"
